<?php
	...
	$informe = $db->query("SELECT equipo_tareas_informe.eti as eti_id,equipo_tareas_informe.fecha as eti_fecha, meses.nombre as mes_nombre, equiposp.nombre as equipo_nombre, equiposp.foto as equipo_foto, equipo_tareas_informe.estado as eti_estado, usuarios.nombres as user_name, usuarios.apellidos as user_lastname,equipo_tareas_informe.observacion as eti_desc   FROM equipo_tareas_informe INNER JOIN meses ON equipo_tareas_informe.mes_id = meses.id INNER JOIN equipo_tareas ON equipo_tareas_informe.eti=equipo_tareas.eti INNER JOIN equiposp ON equipo_tareas.equipo_id=equiposp.id INNER JOIN usuarios ON equipo_tareas_informe.user_id=usuarios.id  WHERE equipo_tareas_informe.eti='$eti'")or die('error'.mysqli_errno($db));
